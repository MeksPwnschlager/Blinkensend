...
IPAddress ip(10,20,30,130);
IPAddress gateway(10,20,30,1);
IPAddress subnet(255,255,0,0);
IPAddress hostIP(10,20,30,245);
...
void loop() {
 int packetLength = UDP.parsePacket();
  if ((bufferSize > packetLength) && (packetLength > 0)){
    UDP.read(buffer, packetLength);
    for (int i=0; i<ledCount; i++){
      int offset = i*3;
      if (frameNumber % 2) {
        strip.setPixelColor(i, buffer[offset], buffer[offset+1], buffer[offset+2]);
      } else {
